...
class Node:
	...
	def createNodeByTimeDate(year, month, day, hour, minute):
		node = Node(address='', year=year, month=Node.__changeDateToInt(month), day=day, hour=hour, minute=minute)
		return node

	def changeDataByData(self, address, year, month, day, hour, minute, tag=False):
		node = Node(address=address, year=year, month=Node.__changeDateToInt(month), day=day, hour=hour, minute=minute, tag=tag)
	...
	def setDate(self, year, month, day):
		date = datetime.date(year=year, month=Node.__changeDateToInt(month), day=day)
		time = self.getTime()
		self.dateTime = datetime.datetime.combine(date=date, time=time)

	def setDateTime(self, year, month, day, hour, minute):
		self.dateTime = datetime.datetime(year=year, month=Node.__changeDateToInt(month), day=day, hour=hour, minute=minute)
